...
if [[ -f "$APPLICATIONS_FILE" ]]; then
    ...
    # Install pacman packages (root)
    if [[ ${#pacman_pkgs[@]} -gt 0 ]]; then
        echo "Installing pacman packages..."
        pacman -S --needed --noconfirm "${pacman_pkgs[@]}"
    fi

    # Install AUR packages with yay as non-root user
...
if [[ -d "$CONFIG_DIR" ]]; then
    # Loop over top-level files/directories in the repo config
...
if [[ -d /etc/greetd ]]; then
    ...
    echo "WARNING: /etc/greetd not found. Did packages install correctly?"
fi
